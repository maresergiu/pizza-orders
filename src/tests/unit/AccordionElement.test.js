...
test("should display the name property in the AccordionElement component", () => {
    ...
    const accordionElement = container.querySelector(".accordion-element_order-name"),
        userName = accordionElement.textContent;

    expect(accordionElement).toBeInTheDocument();
    expect(userName.indexOf(mockedData.accordionElement.firstname) > -1).toBe(true);
    expect(userName.indexOf(mockedData.accordionElement.lastname) > -1).toBe(true);
});
...
test("should render the gender property in the AccordionElement component", () => {
    ...
    const accordionElementEmail = container.querySelector(".accordion-element_details-email");
...
test("should render the phone property in the AccordionElement component", () => {
    ...
    const accordionElementPhone = container.querySelector(".accordion-element_details-phone");
...
test("should render the pizza property in the AccordionElement component", () => {
    ...
    const accordionElementPizza = container.querySelector(".accordion-element_details-pizza");

    expect(accordionElementPizza).toBeInTheDocument();
});

test("should show the details if accordion-element_order-name doesn't have the active class", async () => {
    const { container } = render(
        <Provider store={store}>
            <AccordionElement
                activeAccordion={0}
    ...
    let orderName = container.querySelector(".accordion-element_order-name").className,
...
test("should hide the details if accordion-element_order-name has the active class", async () => {
    const { container } = render(
        <Provider store={store}>
            <AccordionElement
                activeAccordion={-1}
